...
## Step 4: Install, Add the ERD & Start
...
2. Add the live ERD: follow **Installing or refreshing the ERD in an existing mock** in `${CLAUDE_PLUGIN_ROOT}/agents/interface-designer.md` (read the agent file now if it is not loaded yet). Find where the adopted app declares its record types before choosing the `include` path.
3. Determine the dev server command from `package.json` scripts (usually `dev` or `start`)
4. Start the dev server in background
5. Wait for Vite/webpack to be ready
...
3. Navigate to `/erd` on the same port and screenshot the diagram
...
- The ERD URL, its `[erd]` summary, and any link questions still waiting on the user
