...
1. Copy these files from the template, **overwriting** the Vite defaults:
   ...
   - `vite.config.ts`: router plugin, the `erd()` plugin, and the `optimizeDeps.include` list; never add an `optimizeDeps.exclude`
   - `src/entry.ts`
...
2. Copy the live ERD, which the template's `vite.config.ts` already registers:
   ```bash
   cp -R ${CLAUDE_PLUGIN_ROOT}/templates/erd design/{project-name}/erd
   ```
   It serves the entity relationship diagram at `/erd` on the dev server and rewrites `ERD.md` whenever the types in `src/types/` change. The folder is plugin-owned: never edit it in the mock.
...
4. Delete the Vite boilerplate the template replaces (`public/favicon.svg` stays; `index.html` links it):
   ```bash
...
5. Generate the route tree once, so `tsc` can see it before the dev server has ever run:
   ```bash
   cd design/{project-name} && pnpm exec vite build
   ```
   The router plugin writes `src/routeTree.gen.ts` during the build and rewrites it whenever a route file changes. Commit it; never edit it by hand. The same build writes `ERD.md` and prints `[plugin erd] No entity types found`, which is expected until the first entity lands in `src/types/`. The `dist/` output is disposable.
...
3. Navigate to `/erd` on the same port and confirm the ERD page loads (it reads "No entities yet" on a fresh scaffold)
4. Show the screenshot to the user

## Step 8: Ready for Prompts

Tell the user the project is running and ready, and that the live ERD of whatever they build is at `http://localhost:{port}/erd`. They can now describe what they want built.
...
2. **Implement** the changes following the interface-designer agent patterns - Read ${CLAUDE_PLUGIN_ROOT}/agents/interface-designer.md first if not already loaded this session, then follow its mock-service, RHF+Zod, ERD, and changelog conventions:
   - Types in `src/types/`, with references named or tagged so the ERD can link them
...
3. **Verify:**
   ...
   - Take a Chrome DevTools screenshot of the result, plus `/erd` when the prompt changed `src/types/`
...
- **Always keep the ERD clean:** entity types in `src/types/`, no `[plugin erd]` warnings at commit, `ERD.md` committed with the change
